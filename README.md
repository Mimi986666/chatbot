...
基于 Streamlit 的智能聊天系统，整合对话记账、情感陪伴、期末备考规划三大模块，支持 DeepSeek AI 增强智能对话。
...
1. **会话保持**：每个模块独立的对话历史，使用 session_state 持久化
2. **消费数据**：对话记账模块的数据自动保存到 CSV 文件
3. **学习计划**：期末备考模块的计划可一键导出为 TXT 文件
